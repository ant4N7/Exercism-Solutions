...
class StackUnderflowError(Exception):
    """Exception raised when Stack does not have sufficient items."""
...
class Forth:
    ...
    def _is_numeric(token: str) -> bool:
        """Check if token represents a valid integer (including signed integers)."""
        if not token:
            return False
    ...
    @staticmethod
    def _load_defaults() -> dict[str, collections.abc.Callable[[Forth], None]]:
        """Build the default Forth dictionary."""

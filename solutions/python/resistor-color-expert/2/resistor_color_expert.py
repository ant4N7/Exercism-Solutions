...
def resistor_label(colors):
    ...
    *res_bands, zeros, tol_band = colors
    res_bands = ''.join(str(digit) for digit in [RESISTANCE_BAND_LIST.index(i) for i in res_bands])
    ...
    if len(str(resistance)) > 3:
        return f'{resistance/1000} {PREFIX[z+1]}ohms {tolerance}'
    return f'{resistance} {PREFIX[z]}ohms {tolerance}'

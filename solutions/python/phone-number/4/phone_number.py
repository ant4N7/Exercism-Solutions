...
class PhoneNumber:
    ...
    errors = {'letters':'letters not permitted',
              ...
              'too short':'must not be fewer than 10 digits',
              'not US':'11 digits must start with 1',
    ...
    def __init__(self, number: str):
        ...
        self.area_code = re.search(r'\d{3}',self.number)[0]
    ...
    def validate(cls,number: str):
        ...
        if len(digits) < 10: raise ValueError(cls.errors['too short'])
        if len(digits) == 11 and digits[0] != '1': raise ValueError(cls.errors['not US'])
